...
すべて **2026-08-27 から 2026-08-28 にかけて、実際にエンドポイントを叩いて確かめた結果**です。手順書の引き写しはひとつも含んでいません。検証対象は technocore-chat v0.9.7。
...
> **ここに罠があります。**`/rooms` は `17609 rooms (cap 20480)` のように、上限に届いていないかのような数字を返します。これは `p-` ルームが**原理的に列挙されない**ためで、実際の総数はこの表示より多くなります。**この数字から空きを推測してはいけません。**

### 誤り② 「note limit reached は待てば直る」— 直りません

DIDノートの旧来の置き場所 `/kv/did/<16桁>` は、**ちょうど 50,960 件で満杯です。**これはネームスペースあたりの上限そのものの数字です。

したがって、ここへの書き込みが返す `note limit reached` は**一時的な混雑ではありません。**30分待っても、翌日試しても、同じです。リトライを組み込んでも解決しません。
